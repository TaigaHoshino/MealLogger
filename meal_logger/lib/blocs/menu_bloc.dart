...
class MenuBloc {
  ...
  final _removeMealFromTodayMenuController = BehaviorSubject<LoadingState<void>>();
  ...
  Future<void> removeMealFromTodayMenu(Menu menu, Meal meal) async {
    _removeMealFromTodayMenuController.sink.add(const LoadingState.loading(null));

    try {
      await _menuRepository.removeMealFromMenu(menu, meal);
      _removeMealFromTodayMenuController.sink.add(const LoadingState.completed(null));
    }
    on Exception catch (e){
      print(e);
      _removeMealFromTodayMenuController.sink.add(LoadingState.error(e));
    }
    catch (e) {
      print(e);
      _removeMealFromTodayMenuController.sink.add(LoadingState.error(Exception('Unexpected error is occurred')));
    }
  }
